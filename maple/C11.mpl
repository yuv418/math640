#TRA(M): inputs a list M as a list of lists and outputs the transpose matrix
#also a list of lists
with(linalg):

TRA:=proc(M) local i,j,M1:
    [seq([seq(M[j][i], j=1..nops(M))], i=1..nops(M[1]))]
end:
...
########### OLD CODE #############
#C9.txt, Feb. 15, 2024
Help:=proc(): print(` SynT(q,M), WtEnumerator(q,M,t), antiPCM(q,H), DecodeLT(q,M) `): end:
...
CycS:=proc(u) local i:
n:=nops(u):
{seq([op(i..n, u),op(i..i-1,u)])}
end:

# Is the linear code over GF(q) a cyclic code?
IsCC := proc(q,M)
C:=LtoC(q,M);
...
# AllPols: the set of all the polynomials of degree <=d
# in x over GF(q) (q is a prime)
AllPols := proc(q,d,x) local S,s,i:
    ...
    S:=AllPols(q,d-1,x):
    return {seq(seq( s + i*(x^d) ), i=0..q-1), s in S}:
end:

Mul:=proc(q, P1, P2, x, f): rem(P1*P2, f, x) mod q: end:

Irreds:=proc(q,f,x) local d:
    d:=degree(f,x):
    S:=AllPols(q,d-1,x):
    S1:=S minus AllPols(q,0,x):
end:
